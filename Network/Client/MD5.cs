...
namespace Client
{
	public class MD5
	{
		...
		public string Message
		{
			...
			{ 
				string st ;
                char[] tempCharArray = new Char[byteInput.Length];
		...
		public byte [] ValueAsByte
		{
			get
			{
                byte[] bt = new byte[byteInput.Length];
                for (int i = 0; i < byteInput.Length; i++)
                    bt[i] = byteInput[i];
				return bt;
          }
		...
		// Create Padded buffer for pre-processing , buffer is padded with 0 
		...
		protected byte[] CreatePaddedBuffer()
		{
            ...
            bMsg[byteInput.Length] |= 0x80;		//making first bit of padding = 1,
        ...
        public string Calculate_MD5_Value(string input)
        {
            MD5 md;
            md = new MD5();
            md.Message = input;
